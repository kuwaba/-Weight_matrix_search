...
namespace motiflength
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> kin = new List<string>();
            ...
            foreach(string motif_f in motif_fs)
            {
                ...
                foreach (StringBuilder f_m in motifs)
                {
                    ...
                    //if (motif[motif.Count - 1].length != 0)
                    //{
                        //Console.WriteLine(motif[motif.Count-1].title +","+ motif[motif.Count - 1].length);
                        if (kind.ContainsKey(motif[motif.Count - 1].kind.ToString()))
                        {
                            kind[motif[motif.Count - 1].kind.ToString()] = kind[motif[motif.Count - 1].kind.ToString()] + 1;
                        }else
                        {
                            kind[motif[motif.Count - 1].kind.ToString()] = 1;
                        }
                    //}
                    kind.Clear();
                ...
                foreach(string s in kin)
                {
                    if (kind.ContainsKey(s))
                    {
                        kind.Remove(s);
                    }
                }
