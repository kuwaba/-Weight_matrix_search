...
namespace PDB_split
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Dictionary<string, Dictionary<string,struct_site>> struct_site_dic = new Dictionary<string, Dictionary<string, struct_site>>();
            ...
            foreach (string file_f in pdb_fs)
            {
                using (StreamReader r = new StreamReader(file_f))
                {
                    ...
                    struct_site_dic[name] = new Dictionary<string, struct_site>();
                    ...
                    foreach (string cif_part in cif_split)
                    {
                        if( cif_part.IndexOf("_struct_site.") >= 0)
                        {
                            ...
                            if (lines[1].IndexOf("loop") != 0)
                            {
                                ...
                                struct_site_dic[name][struct_site["_struct_site.id"]] = (new struct_site(name,struct_site["_struct_site.id"], struct_site["_struct_site.details"], struct_site["_struct_site.pdbx_evidence_code"]));
                            }
                            else
                            {
                                //foreach (string line in lines)
                                for(int i=0; i< lines.Length;i++)
                                {
                                    ...
                                    if (line.IndexOf("_") != 0 && line.IndexOf("loop") == -1 && line.Trim().Length > 2)
                                    {
                                        ...
                                        if (line_sp.Length >= 3)
                                        {
                                            ...
                                            struct_site_dic[name][line_sp[0]] = (new struct_site(name, line_sp[0], dete.ToString(), line_sp[line_sp.Length - 1]));
                                        }else
                                        {
                                            string id = line.Trim();
                                            i++;
                                            ...
                                            struct_site_dic[name][id] = (new struct_site(name, id,dete.ToString(),pdbx_evidence_code));
                        ...
                        }else if(cif_part.IndexOf("_struct_site_gen.") >= 0)
                        {
                            foreach (string line in cif_part.Split('\n'))
                            {
                                if (line.IndexOf("_") != 0 && line.IndexOf("loop") == -1 && line.Length > 4)
                                {
                                    ...
                                    struct_site_dic[name][line_sp[1]].add_struct_site_gen(ssg);
                                }
                            }
                        }
                        else if (cif_part.IndexOf("_pdbx_poly_seq_scheme.") >= 0)
                        {
                            ...
                            foreach(KeyValuePair<string, struct_site> s_s in struct_site_dic[name])
                            {
            ...
            foreach(KeyValuePair<string, Dictionary<string, struct_site>> s_s in struct_site_dic )
            {
                string name = s_s.Key;
                Console.WriteLine(name);
